...
main (int argc, char **argv)
{
  ...
  fuzziness = g_test_rand_double_range (0, 1);

  /* make an instance */
  tree = tree_instance_new (NULL, 4);

  /* serialise it */
  serialise_tree (tree, &serialised);

  {
    GVariant *test = g_variant_new ("(s@ay)",
                                    g_variant_type_info_get_type_string (serialised.type_info),
                                    g_variant_new_from_data (G_VARIANT_TYPE ("ay"),
                                                             serialised.data,
                                                             serialised.size,
                                                             TRUE, NULL, NULL));
    if (!g_file_set_contents (outpath, g_variant_get_data (test),
                              g_variant_get_size (test), &error))
      goto out;
